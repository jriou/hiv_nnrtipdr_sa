functions {
  // ODE for country-level transmission model --------------------------------------------------------------
    real[] ode(real t,
               real[] y, // [1] S I T
               real[] theta, // [2] beta, tau
               real[] x_r, // dummy
               int[] x_i // dummy
    ) {
      real dydt[5];
      real tau_t = t<x_i[3] ? 0.0 : theta[2]; // progressive introduction of ART using a logistic function
      dydt[1] = - theta[1] * y[1] * y[2];
      dydt[2] = theta[1] * y[1] * y[2]  - tau_t * y[2];
      dydt[3] = tau_t * y[2];
      // dummy: incidence 
...
transformed parameters {
  ...
  for(i in 1:G) beta[i] = beta_raw[i] / p_beta;
  for(i in 1:G) tau[i] = tau_raw[i] / p_tau;
  for(i in 1:G) sigma[i] = sigma_raw[i] / p_sigma;
  // concatenate parameters
  for(i in 1:G) theta[i] = to_vector({beta[i],tau[i]});
  // solve ODE
  ...
  for(i in 1:G) {
    ...
    for(j in 1:L) {
      A_output[i,j] = sum(y[i,j,2:3]) <= 0 ? 0.0 : sum(y[i,j,2:3]) * init_pop[i]; // prevalence (sum comps 2:3)
      B_output[i,j] = y[i,j,3] <= 0 ? 0.0 : y[i,j,3] * init_pop[i]; // treatment (comp 3)
